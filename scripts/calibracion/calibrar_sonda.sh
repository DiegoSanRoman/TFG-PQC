...
if ! nc -z localhost "$PUERTO" 2>/dev/null; then
    ...
    echo "  ./servidor_control_pqc.sh ${PUERTO}"
    echo ""
    exit 1
fi
...
if [ $EXIT_CODE -eq 0 ]; then
    ...
    # Renombrar archivos de resultados
    if [ -f "${OUTPUT_DIR}/resultados_sonda_pqc.json" ]; then
        TIMESTAMP=$(date +%Y%m%d_%H%M%S)
        mv "${OUTPUT_DIR}/resultados_sonda_pqc.json" "${OUTPUT_DIR}/calibracion_${TIMESTAMP}.json"
        echo -e "${BLUE}📁 Resultados guardados:${NC}"
        echo "  JSON: ${OUTPUT_DIR}/calibracion_${TIMESTAMP}.json"
        ...
        if [ -d "${PROJECT_DIR}/venv" ]; then
            ...
            python "${PROJECT_DIR}/scripts/ml/analizar_resultados.py" \
                --input "${OUTPUT_DIR}/calibracion_${TIMESTAMP}.json" \
