...
def sonda_pqc(hostname, group=None):
    ...
    try:
        ...
        if "Cipher is" in stdout or "New, " in stdout:
            ...
            for line in stdout.split('\n'):
                if "Server Temp Key" in line or "Key Exchange" in line:
                    logger.debug("Éxito: %s - %s - %s", hostname, group if group else "Automático", line.strip())
        ...
        logger.debug("Rechazado: %s - %s", hostname, group if group else "Automático")
...
def escanear_servidor_pqc(hostname: str, grupos: List[Optional[str]]) -> Dict[str, Any]:
    '''
    ...
    for g in grupos:
        ...
        resultado = sonda_pqc(hostname, g)
...
if __name__ == "__main__":
    ...
    logger.info("Binario OpenSSL: /opt/openssl/bin/openssl")
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        ...
        futuros = {executor.submit(escanear_servidor_pqc, host, grupos): host for host in hostnames}
